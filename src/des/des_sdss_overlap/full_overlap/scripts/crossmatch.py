...
if __name__ == '__main__':
    ...
    sdss_fulloverlap_dataset_path = PATH+'data/sdss-galaxyzoo/des_overlap/merged_dataset.csv'
    des_ret_sql_query_dataset_path = PATH+'src/des/des_sdss_overlap/full_overlap/metadata/ret_sql_query/'
    out_dir = PATH+'src/des/des_sdss_overlap/full_overlap/metadata/crossmatch/'

    des_overlap_dbs = [f for f in listdir(des_ret_sql_query_dataset_path) if isfile(join(des_ret_sql_query_dataset_path, f))]

    sdss = pd.read_csv(sdss_fulloverlap_dataset_path)
